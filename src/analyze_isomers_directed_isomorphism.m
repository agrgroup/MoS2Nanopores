...
% Required to write MS-Excel files on Ubuntu, comment following lines if using on Windows
...
for j=pore_size_list
    ...
    dirname = ['./catalog_ES_1.355/pore',num2str(j)]
...
[~,length]= size(numIsomers);
count = 1;
for i=1:length
    if (numIsomers(i) > 0)
        store(count)=numIsomers(i);
        count=count+1;
    end
end
store
% Write list of isomers with their respective count to CSV file
csvwrite([dirname,'/pore',num2str(j),'_count.csv'],[pore_size_list;store]);
end